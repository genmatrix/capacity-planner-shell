...
def _payload_lobs(p: dict) -> dict:
    ...
    for lob, d in p["lobs"].items():
        ...
        if "starting_hc_pt" not in _as:
            ...
            # Split the old single headcount by the percentage the plan was
            # saved with — the planner's own number, not a guess — unrounded so
            # the total is preserved exactly. NOT numerically inert on purpose:
            # part-timers leave the attrition walk, which is the requested fix
            # ("part-time barely ever has any attrition"). ft_pct stays in the
            # dict untouched so historical changelog diffs of it remain real.
            _share = min(1.0, max(0.0, float(_as.get("ft_pct", 100.0) or 100.0) / 100))
            _tot = float(_as.get("starting_hc", 0.0) or 0.0)
            _as["starting_hc"] = _tot * _share
            _as["starting_hc_pt"] = _tot * (1 - _share)
...
ASSUMPTION_LABELS = {
    ...
    # Retired 2026-08-13 (split into the FT/PT counts above) but kept so
    # historical diffs of it still read as what they were.
    "ft_pct": "Full-time % (retired)",
...
def compute_plan(lob_data: dict, closures=None) -> pd.DataFrame:
    ...
    # The walk is FULL-TIME ONLY (user 2026-08-13: "part-time barely ever has
    # any attrition"). `starting_hc` is the FT count and `starting_hc_pt` the
    # PT count — typed COUNTS, not a percentage split; the old `ft_pct` model
    # attrited part-timers with everyone else and put drift into Staffed FTE.
    # Transfers and graduating classes are full-time hires, so they land in
    ...
    # starting headcount is PEOPLE, not FTE). Since 2026-08-13 the split is
    # typed COUNTS, not a percentage: `starting_hc_pt` part-timers sit outside
    # the attrition walk and each contributes pt-hours ÷ paid-hours of an FTE.
    # LOA and Mentors come out of the FULL-TIME line — mentors are experienced
    # agents by definition, and a percentage allocation of leave against a
    # non-attriting flat PT count would be precision the inputs don't have.
    #
...
def _default_lob(names: list[str]) -> str:
    ...
    for n in names:
        _a = (lobs.get(n, {}).get("assumptions") or {})
        hc = (float(_a.get("starting_hc", 0) or 0)
              + float(_a.get("starting_hc_pt", 0) or 0))
        if hc > best_hc:
            best, best_hc = n, hc
    return best
...
with st.sidebar:
    ...
    if view != CONSOLIDATED:
        ...
        with st.popover(f"Assumptions — {view}", width="stretch"):
            a["starting_hc"] = st.number_input(
                "Starting HC — full-time", 0.0, 2000.0,
                float(a["starting_hc"] or 0.0), 1.0, disabled=RO,
                key=f"as_hc_{view}",
                help="Full-time production agents on the line at week 1, in "
                     "PEOPLE. Attrition, transfers and graduating classes all "
                     "walk THIS number.")
            a["starting_hc_pt"] = st.number_input(
            ...
            a["pt_hours_per_week"] = st.number_input(
                ...
                help="What a part-time agent is paid for in an average week. "
                     "Applied to the **Starting HC — part-time** count: each "
                     "part-timer contributes this ÷ paid hours of an FTE to "
                     "Staffed FTE, Net FTE and Volume Capacity (the gap shows "
...
def attrition_trend_frame(plan: pd.DataFrame, roster: pd.DataFrame,
                          line: str = "all") -> pd.DataFrame | None:
...
def _line_arrays(plan: pd.DataFrame, roster: pd.DataFrame, line: str):
    """(recorded with NaN where blank, modelled, start-of-week HC, transfers
    out) for one line — no 'nothing recorded' judgement, so lines can be
    summed before that call is made."""
    ...
    if line == "ft":
        start = pd.to_numeric(plan["Prod HC — FT (start)"], errors="coerce").fillna(0).to_numpy(dtype=float)
        xo = (np.clip(-pd.to_numeric(roster["Transfers +/-"], errors="coerce")
                      .fillna(0).to_numpy(dtype=float), 0.0, None)
              if "Transfers +/-" in roster.columns else np.zeros(len(act)))
    else:   # start-of-week PT = end + departures − hires; PT has no transfers
        ...
        xo = np.zeros(len(act))
    return act, mod, start, xo


def _attrition_weekly(plan: pd.DataFrame, roster: pd.DataFrame, line: str = "all"):
    """(recorded, modelled, start-of-week HC, last recorded index, transfers
    out) — or None when nothing was recorded. `line` is "ft", "pt" or "all".
    **"all" is what the pages show (user 2026-09-18: "combine PT and FT, PT
    attrition is not modelled")**: one series, departures from both lines
    against the modelled figure of both (PT's is 0 at the default PT rate),
    on the combined start-of-week headcount. A week counts as recorded when
    EITHER line has a figure."""
    if line == "all":
        parts = [x for x in (_line_arrays(plan, roster, "ft"), _line_arrays(plan, roster, "pt"))
                 if x is not None]
        if not parts:
            return None
        acts = np.vstack([x[0] for x in parts])
        any_rec = np.isfinite(acts).any(axis=0)
        if not any_rec.any():
            return None
        last = int(np.flatnonzero(any_rec).max())
        act = np.where(any_rec, np.nansum(acts, axis=0), 0.0)
        mod = sum(x[1] for x in parts)
        start = sum(x[2] for x in parts)
        xo = sum(x[3] for x in parts)
        return act, mod, start, last, xo
    arr = _line_arrays(plan, roster, line)
    if arr is None:
        return None
    act, mod, start, xo = arr
...
def _monthly_rows(weeks: list[str], rec: np.ndarray, mod: np.ndarray, hc: np.ndarray,
    ...
    figure is summed over the SAME weeks, so a month with two of four weeks
    recorded compares two against two and says so. Rates are Σ departures ×
    52 ÷ Σ start-of-week headcount over the counted weeks — the sidebar's
    measured definition, so the two agree. Assumed %/yr is derived the same
    way from the modelled departures, which makes the consolidated figure a
...
def attrition_by_month(plan: pd.DataFrame, roster: pd.DataFrame,
                       line: str = "all") -> pd.DataFrame | None:
    """One LOB's month-over-month recorded vs modelled departures to date."""
    w = _attrition_weekly(plan, roster, line)
    if w is None:
        return None
...
def attrition_by_month_consolidated(lobs: dict, line: str = "all") -> pd.DataFrame | None:
    """Org-wide: departures and headcount summed across LOBs week by week,
    each LOB counted through ITS OWN last recorded week (a line that stopped
    recording in June contributes nothing to August rather than zeros)."""
    n = None
    tot_r = tot_m = tot_h = None
    weeks = None
    last_any = -1
...
def _render_month_table(df: pd.DataFrame, key: str):
    ...
    st.download_button("Download month-over-month CSV",
                       ...
                       file_name=f"attrition_by_month_{key}.csv", mime="text/csv",
                       key=f"dl_attr_month_{key}")


def render_attrition_trend(plan: pd.DataFrame, roster: pd.DataFrame, view: str):
    """Capacity Plan → 'Attrition — recorded vs modelled': ONE series (FT + PT
    combined — user 2026-09-18), cumulative from week 1, with the headline gap
    in words, then the month-over-month table."""
    import altair as alt
    df = attrition_trend_frame(plan, roster, "all")
    with brand.section("attr_trend", "Attrition — recorded vs. modelled",
                       "Cumulative departures from week 1, full-time and part-time "
                       "together. The dashed line is what the assumed rate takes each "
                       "week on the real headcount; the solid line is what you recorded "
                       "PLUS transfers out of this line (negative Transfers +/-). Where "
                       "they part is where the rate assumption is wrong."):
        if df is None:
        ...
        brand.chart(ch + _tr if _tr is not None else ch, height=220)
...
def _org_measured_attrition(plans: dict) -> dict | None:
    ...
    for name, d in st.session_state.lobs.items():
        ...
        if not m.any():
            continue
        hc = plans[name]["Prod HC — FT"].to_numpy(dtype=float)   # PT never attrites
...
if page == "Executive View":
    ...
    with brand.section("attr_month_all", "Attrition — month over month, all LOBs",
                       "Recorded vs modelled departures, full-time and part-time together, "
                       "summed across every line, each counted through its own last "
                       "recorded week. Recorded = departures "
                       ...
                       "assumptions."):
        _mm = attrition_by_month_consolidated(st.session_state.lobs, "all")
        if _mm is not None and not _mm.empty:
            _render_month_table(_mm, "all")
        else:
            st.caption("No departures recorded on any line yet — enter them week by week "
                       "in each LOB's roster grid and the monthly view appears here.")
else:
